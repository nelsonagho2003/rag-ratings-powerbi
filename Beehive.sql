...
WITH cte AS (
    SELECT
        ...
        c1.SNAPSHOT_PERIOD
    FROM USER_TAB_OWNER.UT_PEOPLE_BS_CASELOAD_RAG AS c1
    LEFT OUTER JOIN (
        SELECT DISTINCT 
            WBT, 
            COND_RAG 
...
SELECT 
    *,
    LAG(student_count, 1) OVER (
        PARTITION BY work_based_tutor, condition_rag
        ORDER BY CONVERT(DATE, '01 ' + SNAPSHOT_PERIOD, 113)
    ) AS student_count_previous_month
FROM cte
ORDER BY work_based_tutor, condition_rag, CONVERT(DATE, '01 ' + SNAPSHOT_PERIOD, 113);
